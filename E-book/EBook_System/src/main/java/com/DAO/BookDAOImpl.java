...
public class BookDAOImpl implements BookDAO {
	...
	public boolean editImageUpdate(bookDtls book) {
		boolean f = false;
		try {
			String sql = "update book set photo=? where bookId=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, book.getBookImg());
	...
	public boolean oldBookDelete(String email, String cat) {
		...
		boolean f = false;
		try {
			String sql = "delete from book where user_email=? and bookCategory=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, email);
			ps.setString(2, cat);
			int i = ps.executeUpdate();
